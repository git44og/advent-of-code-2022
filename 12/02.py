...
class Step:
    ...
    def __init__(self, previous, num, x, y, map):
    ...
    def getNext(self) -> list:
        ...
        for neighbour in neighbours:
            ...
            if nextHeight - currentHeight >= -1:
                next.append(Step(self, self.num+1, neighbour['x'], neighbour['y'], self.map))
        if len(next) > 0:
            pos = '{}-{}'.format(self.x, self.y)
            visited.append(pos)
...
print (start)
print (goal)
step = Step(None, 0, goal['x'], goal['y'], map)
current = [step]
round = 0
